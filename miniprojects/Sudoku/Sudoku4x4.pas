...
Type
  T4Pencil = Array[0..15] Of Boolean;
  ...
  T4Field = Array[0..15] Of Array[0..15] Of T4subfield;

Procedure ClearField(Var Value: T4field);
Procedure New4Field(Var Value: T4field; Technik: Integer);
Procedure Mark(Var Value: T4field; Number: integer);
Procedure GetPencil(Var Value: T4field);
Procedure Solve4(Var Value: T4field; Step: boolean; Technik: Integer);
Function isready4(Value: T4field): boolean;
Function isreadyUser4(Value: t4field; Allowmessage: boolean): boolean;
Function Sudoku4Solvable(Value: T4field): Boolean;
...
Procedure Mark(Var Value: T4field; Number: integer);
  ...
  Begin
    ...
    For xx := x - x Mod 4 To x - x Mod 4 + 3 Do
      For yy := y - y Mod 4 To y - y Mod 4 + 3 Do
        Value[xx, yy].marked := True;
    // Markeiren Waagrecht, Senkrecht
    For xx := 0 To 15 Do Begin
      ...
      For y := 0 To 3 Do Begin
        ...
        For x1 := 0 To 3 Do
          For y1 := 0 To 3 Do Begin
            If Penc[Value[x * 4 + x1, y * 4 + y1].value - 1] Then erg := false;
            Penc[Value[x * 4 + x1, y * 4 + y1].value - 1] := true;
          End;
      End;
  ...
  For z := 1 To 16 Do Begin
    ...
    For y := 0 To 3 Do
      For x := 0 To 3 Do Begin
        ...
        For y1 := 0 To 3 Do
          For x1 := 0 To 3 Do
            If (Not Value[x * 4 + x1, y * 4 + y1].marked) Or (Value[x * 4 + x1, y * 4 + y1].Value = z) Then sm := true;
...
Procedure SolveTry(Var Value: T4field);
Label
  raus;
Var
  actual: T4field;
...
Begin
  ...
  While stack <> Nil Do Begin
    ...
    While (y < 16) And Not sm Do Begin
      x := 0;
      While (x < 16) And Not sm Do Begin
      ...
      If Not sm Then Begin
        ...
        If (y = 16) And Not sm2 Then Begin
          y := 0;
    ...
    If sm And Sudoku4Solvable(actual) Then Begin
      For z := 15 Downto 0 Do
        If Actual[x, y].pencil[z] Then Begin
          Actual[x, y].value := z + 1; // Setzen des Wertes
          //          Solve4(actual, false, 511); Erzeugt Fehler
                    // Das Ende Der Breitensuche
          If Isready4(actual) Then Begin
            Clear;
            Value := Actual;
          End
          Else Begin
...
Function HiddenSingle(Var Value: T4field; Step: Boolean): Boolean;
Label
  Raus;
Var
  ...
  While z < 17 Do Begin
    ...
    For xx := 0 To 3 Do
      For yy := 0 To 3 Do Begin
        ...
        For x := 0 To 3 Do
          For y := 0 To 3 Do
            If Not Value[xx * 4 + x, yy * 4 + y].marked Then Begin
              xn := xx * 4 + x;
              yn := yy * 4 + y;
              inc(n);
            End;
...
Function pencilcount(Value: T4pencil): integer;
Var
  z, erg: integer;
Begin
  erg := 0;
...
Function NakedSingle(Var Value: T4field; Step: Boolean): Boolean;
Label
  raus;
Var
  weiter: boolean;
...
Procedure GetSolvable4Sudoku(Var Data: T4field; Technik: Integer);
Const
  MaxFehlercount = 25;
Var
  x, y, z: Integer;
  ...
  f: T4field;
Begin
  If isready4(Data) Then Begin // zuerst mus geschaut werden ob das Rätsel überhaupt Komplett ist, sonst haben wir eine Endlosschleife
    ...
    While weiter Do Begin
      ...
      Data[x, y].value := 0;
      // Lösen des Rätsels
      solve4(data, false, Technik);
      // Schaun ob es lösbar war
      If Not isready4(Data) Then
...
Begin
  ...
  For n := 1 To Numbercount Do Begin
    ...
    While Value[x, y].marked Or (Value[x, y].value <> 0) Do Begin
      ...
      x := random(16);
      y := random(16);
      If zwangsabbruch Or Not Sudoku4Solvable(Value) Then Goto raus;
    End;
    Value[x, y].value := z;
    Value[x, y].fixed := True;
    If Not Sudoku4Solvable(Value) Then Goto Rein;
  End;
  If Not Sudoku4Solvable(Value) Then Goto Rein;
  ...
  If Value[0, 0].value <> 0 Then Begin
    GetSolvable4Sudoku(Value, Technik);
  End;
  Raus:
