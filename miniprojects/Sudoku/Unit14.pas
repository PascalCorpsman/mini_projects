...
Uses
  ...
  StdCtrls, printers, LResources, Sudoku5x5;
...
Procedure Drucke(rect: Trect; sudo: T5field);
Var
  breite: integer;
...
Begin
  ...
  breite := min(abs(rect.Left - rect.Right) Div 27, abs(rect.top - rect.Bottom) Div 27);
  // Ermitteln des Offset für die Höhe und Breite
  xo := -breite Div 4 + rect.left;
  yo := round((abs(rect.top - rect.Bottom) - (Breite * 25.5)) / 2) + rect.top;
  Textsize := 1;
    ...
    For yy := 0 To 4 Do
      For x := 0 To 4 Do
        For y := 0 To 4 Do Begin
          ...
          p.canvas.rectangle(xo + breite + x * breite + round(XX * breite * 5.25), yo + y * breite + round(yy * (breite * 5.25)), xo + Breite + (x + 1) * breite + round(XX * breite * 5.25), yo + (y + 1) * breite + round(yy * breite * 5.25));
          // Malen einer Zahl
          If Sudo[xx * 5 + x, yy * 5 + y].Value <> 0 Then Begin
            p.canvas.Font.Size := Textsize;
            p.canvas.textout((breite - P.canvas.TextWidth(inttostr(Sudo[xx * 5 + x, yy * 5 + y].Value))) Div 2 + xo + breite + x * breite + round(XX * breite * 5.25), yo + (breite - P.canvas.Textheight(inttostr(Sudo[xx * 5 + x, yy * 5 + y].Value))) Div 2 + y * breite + round(yy * (breite * 5.25)), inttostr(Sudo[xx * 5 + x, yy * 5 + y].value));
          End;
        End;
End;
...
Begin
  ...
  If Drucken Then Begin
    ...
    For I := 1 To Stop Do Begin
      // Start Druckauftrag
      // Drucken des Feldes
      Drucke(rect(0, 0, p.Pagewidth, p.Pageheight), fivefield);
